...
func (this InfluxdbClient) Call(items []*cmodel.InfluxdbItem) error {
...
func CreateInfluxdbConnPools(maxConns, maxIdle, connTimeout, callTimeout int, cluster []InfluxdbConnection) *InfluxdbConnPools {
	cp := &InfluxdbConnPools{M: make(map[string]*connp.ConnPool), MaxConns: maxConns, MaxIdle: maxIdle,
	...
	for _, influxdbConn := range cluster {
		...
		cp.M[address] = createOneInfluxdbPool(address, influxdbConn, ct, int32(maxConns), int32(maxIdle))
	}
...
// 同步发送, 完成发送或超时后 才能返回
func (this *InfluxdbConnPools) Call(addr string, items []*cmodel.InfluxdbItem) error {
...
func (this *InfluxdbConnPools) Get(address string) (*connp.ConnPool, bool) {
	this.RLock()
	defer this.RUnlock()
...
func createOneInfluxdbPool(name string, influxdbConn InfluxdbConnection, connTimeout time.Duration, maxConns int32, maxIdle int32) *connp.ConnPool {
	p := connp.NewConnPool(name, influxdbConn.Address, maxConns, maxIdle)
